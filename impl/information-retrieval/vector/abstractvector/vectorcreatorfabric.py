...
class VectorCreatorFabric(object):
    ...
    def get_vector(self, document_id):
        '''x.get_vector(document_id) -> DocumentVector -- either returns a buffered vector or creates a new one (depending on former queries) that represents the document identified by document_id'''
    ...
    def _create_vector(self, document_id):
        ''' x._create_vector(document_id) -> DocumentVector -- creates a vector that represents the document identified by the document_id'''
        raise NotImplementedError()

    def flush(self):
        '''x.flush() -- removes all buffered vectors.'''
        del(self.cache)
        self.cache = {}
        pass
