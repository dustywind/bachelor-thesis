...
Since the theory has been described in section~\ref{sec:tfidf}, the next big step is to show how vectors for this thesis have been built.
Currently all products are described through their terms.\\
...
For each vector necessary (tf, idf, tf-idf) one can build one vector creator which shares the design of the other vector creators.
Therefore the abstract class \textit{AbstractVectorCreator} has been introduced.
The \textit{AbstractVectorcreator} offers 
