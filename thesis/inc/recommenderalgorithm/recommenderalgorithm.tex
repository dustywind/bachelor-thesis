...
Effectively Rocchio's algorithm creates a vector that can be inserted into the vector space section~\ref{vectorspacemodel} and with each application of Rocchio's algorithm the output vector leads to a point resembling the users preferences.
...
While it is certainly possible its debateable whether this tweak is useful or not, i.e. is there a useful scenario for doing so.
...
The theorie of vector space classification basically states that items within a vector space model form regions with the items next to them and these different regions do not overlap.
...
This can be used to find items which are near a given user vector since they should have similiar characteristics as the user vector.
The more similiar a user vector and a item are, the more likely the item provides use for the user.
\citep[p.~298-291]{manning:2009}
\\
...
This behaviour can be used for identifying items which can be recommendet to a user as queried in \textbf{Q5}.
