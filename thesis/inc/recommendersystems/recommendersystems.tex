...
%In a e-commerce context for example, these items can be a selection of products an online shop has.
...
There are four major approaches \citep[p.~378]{burke:2007} which will be discussed and adapted to the example of an online clothing shop as the final objective is to implement one.

\paragraph{Content-based}
\index{content-based}
Each item or product is defined by its attributes such as its colour, price, age, etc.
And each user has his own rating for each of the attributes which determine whether he likes an item or not.
Every time the user gives feedback to the RS in form of preferring or rejecting an item the RS will adapt its information about the users preferences - the user's rating of attributes will be adjusted.
...
The closer all attributes of an item match the preference of a user, the higher the probability that the user likes it.
...
Every new user who has no purchase history or anything else indicating his preferences leads to a \index{cold-start problem} cold-start problem.
...
\citep[p.~378-379]{burke:2007}
%Similar to the content-based approach these RS require information about customer preferences in order to find users with similiar behaviour and likings.
...
Knowledge-based techniques however are not, but have other difficulties.
